...
const i = run(['probe-prototype-keys.js']);
const j = run(['probe-relationship-polarity.js']);
const k = run(['probe-duplicate-cards.js']);
const l = run(['probe-undo.js']);
const m = run(['probe-explanation-length.js']);
console.log('\n================================');
...
console.log('prototype keys   : ' + (i ? 'HOLDS' : 'FAIL'));
console.log('rel. polarity    : ' + (j ? 'HOLDS' : 'FAIL'));
console.log('duplicate cards  : ' + (k ? 'HOLDS' : 'FAIL'));
console.log('undo handling    : ' + (l ? 'HOLDS' : 'FAIL'));
console.log('explanation len  : ' + (m ? 'HOLDS' : 'FAIL'));
process.exitCode = a && b && c && d && e && f && g && h && i && j && k && l && m ? 0 : 1;
